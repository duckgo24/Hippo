...
export default function CallScreen() {
    ...
    const { data: userCall, isLoading: isFetchAccountLoading, isSuccess: isFetchAccountSuccess } = useQuery({
    ...
    }, [isFetchAccountSuccess, isAcceptCall, socket])
    ...
    return (
        ...
        <div className="flex flex-col items-center justify-center h-screen">
            {isOnCam &&
                <video ref={videoRef} className='w-[240px] h-[240px] absolute -top-[16px] left-10 rounded-3xl' autoPlay />
            }
            <div className='w-1/2 h-1/2 flex flex-col items-center'>
                ...
                <p className='font-bold text-xl mt-5'>
                    {userCall?.acc_id !== my_account?.acc_id && `Đang gọi tới ${userCall?.full_name}...`}
                </p>

                {isAcceptCall && <p className="text-gray-500 text-lg">{formatTime(time)}</p>}
                ...
                <div className='flex items-center gap-3 mt-10'>
                    ...
                    <button className='hover:opacity-60'
                        onClick={isAcceptCall ? handleCloseCall : handleCancelCall}>
