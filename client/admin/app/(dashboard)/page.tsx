...
export default function DashBoard() {
    const [timeOption, setTimeOption] = useState<'day' | 'week' | 'month' | 'range'>('day');
    ...
    const { data: accountsOnlineData, isSuccess: isFetchAccountOnlineSuccess, refetch: refetchAccountOnline } = useQuery({
    ...
    const [
        statistalcalCardDataAccount, statistalcalCardDataPost, statistalcalCardDataVideo,
        statistalcalChartDataAccount, statistalcalChartDataPost, statistalcalChartDataVideo,
    ] = useQueries({
        queries: [
            {
            ...
            },
            {
                ...
                queryFn: () => statisticalService.getNumAccountByTime(start_date, end_date),
                enabled: !!start_date && !!end_date && !!Cookie.get("access_token"),
            },
            {
                queryKey: ['get-num-post-by-time', start_date, end_date],
                queryFn: () => statisticalService.getNumPostByTime(start_date, end_date),
                enabled: !!start_date && !!end_date && !!Cookie.get("access_token"),
            },
            {
                queryKey: ['get-num-video-by-time', start_date, end_date],
                queryFn: () => statisticalService.getNumVideoByTime(start_date, end_date),
                enabled: !!start_date && !!end_date && !!Cookie.get("access_token"),
            }
                ...
                <div className='flex items-center justify-end gap-3 bg-white py-3 px-5'>
                    <div className={`${timeOption === 'range' ? 'flex' : 'hidden'} items-center gap-2`}>
                        <div className="relative">
                            ...
                            <input name="start" type="date" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-1.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date start" />
                        </div>
                    ...
                    <div className='text-sm px-3 py-1 flex flex-row justify-end items-center gap-2 bg-slate-300 rounded'>
                        <button className={`${timeOption === 'day' && 'bg-white'} py-1 px-3 rounded-lg`} onClick={handleChangeTimeByDay}>Hôm nay</button>
                        <button className={`${timeOption === 'week' && 'bg-white'} py-1 px-3 rounded-lg`} onClick={handleChangeTimeByWeek}>Tuần này</button>
                        <button className={`${timeOption === 'month' && 'bg-white'} py-1 px-3 rounded-lg`} onClick={handleChangeTimeByMonth}>Tháng này</button>
                        <button onClick={handleToggleMoreOptionTime}>
                            <IoMdMore />
                            <Popper
                                open={openTimePicker}
                                anchorEl={archerElMore}
                                placement='bottom-end'
                            >

                                <div className='flex flex-col items-start bg-white shadow-lg'>
                                    <button className='hover:bg-slate-300 w-full text-start py-1 px-2'>Năm nay</button>
                                    <button
                                        onClick={() => setTimeOption('range')}
                                        className='hover:bg-slate-300 w-full text-start py-1 px-2'
                                    >Mốc thời gian</button>
                                </div>
                            </Popper>
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-6 py-2">
                    <RenderWithCondition condition={timeOption === 'day'}>
                        <div className="flex items-center justify-center w-full p-5 bg-white rounded-lg shadow-lg">
                            <ChartBar chartData={{
                                title: `Biểu đồ số lượng tài khoản ${timeOption === 'day' ? 'hôm nay' : start_date + ' đến ' + end_date}`,
                        ...
                        <div className="flex items-center justify-center w-full p-5 bg-white rounded-lg shadow-lg">
                            <ChartBar chartData={{
                                title: `Biểu đồ số lượng bài đăng ${timeOption === 'day' ? 'hôm nay' : start_date + ' đến ' + end_date}`,
                        ...
                        <div className="flex items-center justify-center w-full p-5 bg-white rounded-lg shadow-lg">
                            <ChartBar chartData={{
                                title: `Biểu đồ số lượng video ${timeOption === 'day' ? 'hôm nay' : start_date + ' đến ' + end_date}`,
